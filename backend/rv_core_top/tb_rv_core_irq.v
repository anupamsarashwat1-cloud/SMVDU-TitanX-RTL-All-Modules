...
// SMVDU-TitanX SoC — rv_core_top Interrupt Preemption Testbench
//
// Structural twin of tb_rv_core_irq running scripts/gen_irq_test.py's
// timing-independent preemption program. The TB raises irq_m_timer at a
// fixed early cycle and HOLDS it; the core must take the machine-timer
// interrupt exactly once at whatever instruction boundary is live,
// commit nothing for the killed beat (including no store side effects),
// run the handler (which clears mie.MTIE first), and resume via mret.
//
// shadow[26] (mepc captured at entry) is TIMING-DEPENDENT — it is
// range-checked against meta[3] instead of compared exactly.
//
// Expectation files load repo-root-relative: backend/rv_core_top/tb_irq_*.
`timescale 1ns/1ps

module tb_rv_core_irq();
    ...
    reg [63:0] meta     [0:3];   // [3] = min legal epc
    ...
    initial begin
        $dumpfile("tb_rv_core_irq.vcd");
        $dumpvars(1, tb_rv_core_irq);     // top-level ports
        ...
        $readmemh("backend/rv_core_top/tb_irq_imem.hex", imem);
        $readmemh("backend/rv_core_top/tb_irq_expected_regs.mem", exp_regs);
        $readmemh("backend/rv_core_top/tb_irq_expected_maddr.mem", exp_maddr);
        $readmemh("backend/rv_core_top/tb_irq_expected_mval.mem", exp_mval);
        $readmemh("backend/rv_core_top/tb_irq_expected_meta.mem", meta);
        if (^imem[0] === 1'bx) begin
            fail("program hex did not load — run scripts/gen_irq_test.py");
        ...
        // Machine timer interrupt: raised while mainline is inside the
        // DIVU stall window, held high for the rest of the run. The
        // handler clears mie.MTIE so entry count stays exactly one.
        fork
            begin : timer_irq_drive
                wait (uut.u_execute.u_csr.mstatus_q[3] === 1'b1);
        ...
        begin : chk_rf
            ...
            for (r = 0; r < 32; r = r + 1) begin
                if (r == 26) begin
                    // mepc saved at trap entry depends on which beat the
                    // interrupt killed — only a floor is architectural.
                    if (shadow[26] < meta[3]) begin
                        $display("FAIL [%0t] x26 epc=0x%016X below min=0x%016X",
                                 $time, shadow[26], meta[3]);
                        rf_err = rf_err + 1;
                    end
                end else if (shadow[r] !== exp_regs[r]) begin
        ...
        if (error_count == 0) begin
            $display("RV_CORE_IRQ VERDICT: ✅ PASS — program retired correctly");
            $display("REGRESS_RESULT: PASS");
        end else begin
            $display("RV_CORE_IRQ VERDICT: ❌ FAIL — %0d errors", error_count);
            $display("REGRESS_RESULT: FAIL");
        end
        $display("==============================\n");
        $finish;
    ...
    initial begin
        ...
        $display("RV_CORE_IRQ VERDICT: ❌ FAIL — watchdog");
        $display("REGRESS_RESULT: FAIL");
        $finish;
    end
endmodule
