...
// SMVDU-TitanX SoC — rv_core_top ISA Compliance Testbench (vs qemu)
//
// Structural twin of tb_rv_core_compliance running scripts/gen_compliance.py's
// pseudo-random RV64IM program (seed-fixed), assembled by the GNU
// toolchain and executed IDENTICALLY under qemu-riscv64 user mode.
// Golden expectations are qemu's architectural state: all of x5..x31
// plus a 1 KiB scratch window. Any mismatch is an RTL ISA bug.
//
// Expectation files load repo-root-relative:
// backend/rv_core_top/tb_compliance_*.
`timescale 1ns/1ps

module tb_rv_core_compliance();
    ...
    initial begin
        $dumpfile("tb_rv_core_compliance.vcd");
        $dumpvars(1, tb_rv_core_compliance);     // top-level ports
        ...
        $readmemh("backend/rv_core_top/tb_compliance_imem.hex", imem);
        $readmemh("backend/rv_core_top/tb_compliance_expected_regs.mem", exp_regs);
        $readmemh("backend/rv_core_top/tb_compliance_expected_maddr.mem", exp_maddr);
        $readmemh("backend/rv_core_top/tb_compliance_expected_mval.mem", exp_mval);
        $readmemh("backend/rv_core_top/tb_compliance_expected_meta.mem", meta);
        if (^imem[0] === 1'bx) begin
            fail("program hex did not load — run scripts/gen_compliance.py");
        ...
        begin : run
            integer cyc;
            // div-heavy random mix needs a longer horizon than the base TB
        ...
        if (error_count == 0) begin
            $display("RV_CORE_COMPLIANCE VERDICT: ✅ PASS — program retired correctly");
            $display("REGRESS_RESULT: PASS");
        end else begin
            $display("RV_CORE_COMPLIANCE VERDICT: ❌ FAIL — %0d errors", error_count);
            $display("REGRESS_RESULT: FAIL");
        end
        $display("==============================\n");
        $finish;
    ...
    initial begin
        ...
        $display("RV_CORE_COMPLIANCE VERDICT: ❌ FAIL — watchdog");
        $display("REGRESS_RESULT: FAIL");
        $finish;
    end
endmodule
